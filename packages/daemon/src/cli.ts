...
const claudeCodeAdapter: Adapter = {
  ...
  install: notImplementedInstall("claude-code"),
  uninstall: notImplementedUninstall("claude-code"),
  doctor: notImplementedDoctor("claude-code"),
};
...
const cursorAdapter: Adapter = {
  ...
  install: notImplementedInstall("cursor"),
  uninstall: notImplementedUninstall("cursor"),
  doctor: notImplementedDoctor("cursor"),
};
...
function notImplementedInstall(surface: string) {
  return async (_opts: InstallOpts): Promise<InstallResult> => ({
    status: "not-implemented",
    message: `install for '${surface}' not yet implemented`,
  });
}

function notImplementedUninstall(surface: string) {
  return async (_opts: { dryRun: boolean }): Promise<UninstallResult> => ({
    status: "not-implemented",
    message: `uninstall for '${surface}' not yet implemented`,
  });
}

function notImplementedDoctor(surface: string) {
  return async (): Promise<DoctorResult> => ({
    status: "not-implemented",
    message: `doctor for '${surface}' not yet implemented`,
  });
}
